...
export class ClaudeRunner extends EventEmitter implements IAgentRunner {
	...
	private async startWithPrompt(
		...
		try {
			...
			//
			// Skipped entirely when `unrestrictedFilesystemAccess` is set: that
			// mode intentionally grants the session read/write access to the
			// whole host (e.g. the Feishu full-access front door).
