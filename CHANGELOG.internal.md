...
### Removed
- Dead-code cleanup for the unified Session · multi-Channel effort (IN-42 §5 P6). Removed the session/entry **storage** half of `SessionCorrelationRegistry` (formerly `GlobalSessionRegistry`) — `createSession`/`getSession`/`updateSession`/`deleteSession`/`getAllSessions`/`addEntry`/`getEntries`/`updateEntry`/`getChildSessionIds`/`restoreState`/`cleanup`, the now-unused `unbind`/`getChannelKeysForSession`, the `EventEmitter` base and its `sessionCreated`/`sessionUpdated`/`sessionCompleted` events, and the `sessions`/`entries` maps. These were never wired into production: `AgentSessionManager` is the single source of truth for session objects, and the registry only ever used its child→parent map (`setParentSession`/`getParentSessionId`) and channelKey→sessionId index (`bind`/`resolve`). `serializeState` is slimmed to those two maps, and `EdgeWorker.restoreMappings` already rebuilt them directly via `setParentSession`/`bind` (never `restoreState`). No runtime behavior change. **Scope note:** the design's P6 also called for removing legacy `event` handler branches "taken over by the bus", but that removal was intentionally **deferred** — the `InternalMessage` bus is still default-shadow (`CYRUS_BUS_SESSION_OWNERSHIP` off by default; Feishu/Slack/GitHub/GitLab and Linear stop/content-update/unassign have no active bus execution path yet), and only Linear issue-state-change/deletion is truly bus-owned (already clean early-returns with nothing dead behind them). Deleting the still-primary legacy branches would break the default runtime, so legacy removal waits until the bus is promoted to active-by-default and validated across all channels. ([IN-51](https://linear.app/principle-intl/issue/IN-51), [#24](https://github.com/yuanze-dev/cyrus/pull/24))

### Added
- Generalized the Feishu completion-only notice into a **process backflow** (IN-42 §Q4 / §5 P4, the final stage). A new `FeishuBackflowSink` plugs into `AgentSessionManager` as a passive `ActivityObserver` (a second, read-only tap alongside the per-session primary `IActivitySink` — it's offered a copy of every posted activity *after* it lands on Linear and can neither block nor fail that path). It reflects only **milestones** back into the originating Feishu thread — turn-final `response` and `error` activities — so the thread gets the answer, not every thought/action/ephemeral tool call. Three guards keep it readable and loop-safe: a **milestone filter** (drops `thought`/`action`/`elicitation`/ephemeral), **idempotency** (each milestone maps to a stable key like `response:<hash>` / `state:<type>`, posted at most once per session, bounded to the last 64 keys per session), and a **throttle** (identical text to the same session inside a 5s window collapses to one post). Re-ingestion is prevented upstream: the sink reuses `postFeishuThreadNotice`, which replies as the bot, so the Feishu translator's self-author filter discards it. Terminal state changes are generalized too — `FeishuIssueNotificationService.notifyIssueCompleted` becomes `notifyIssueStateChange({stateType})` (the completed case keeps a backward-compatible wrapper), and `handleIssueStateChange` now also notifies on `canceled` when backflow is on (idempotent via the persisted `notifiedAt` stamp). The thread binding is resolved from the session's `feishu` `ChannelBinding` overlay added by the P3 cross-channel work. Gated behind a new `CYRUS_FEISHU_BACKFLOW` env flag (off by default; `on`/`true`/`active`/`all`/`feishu` to enable); the sink's live `isEnabled()` read means flipping it off instantly reverts to the exact legacy completion-only notice with no re-registration. ([IN-49](https://linear.app/principle-intl/issue/IN-49))
- Cross-channel prompt injection + per-session serial queue (IN-42 §5 P3, the core-value stage). A Feishu thread follow-up can now steer the Linear agent session its issue was created from, instead of spawning a disconnected chat session. New `SessionSerialQueue` chains every incoming prompt for a given logical session id onto that session's tail, so two channels prompting the same session concurrently can never both observe "runner idle" and start competing turns. When a thread resolves — via the persisted `SessionCorrelationRegistry` — to a session the chat handler doesn't own, `ChatSessionHandler` hands it to `EdgeWorker.injectCrossChannelPrompt`, which (1) runs an **authorization guard first** — only the Feishu chat that originally created the issue may inject (越权 red-line), (2) leaves a Linear-side thought activity ("来自飞书 <用户> 的追问：…") so reviewers see where the prompt came from, and (3) injects through the exact same streaming/`--continue`-resume logic Linear comments use (`handlePromptWithStreamingCheck`), reusing the session's worktree. On `AgentSessionCreated` for a Feishu-originated issue the thread key is bound to the new Linear session so later follow-ups route to it. Gated behind a new `CYRUS_CROSS_CHANNEL_INJECTION` env flag (off by default; `on`/`true`/`active`/`all`/`feishu` to enable) so it's opt-in and instantly reversible, mirroring the `CYRUS_BUS_SESSION_OWNERSHIP` staging. ([IN-48](https://linear.app/principle-intl/issue/IN-48), [#20](https://github.com/yuanze-dev/cyrus/pull/20))
- Activated the unified `InternalMessage` bus as the real session entry point (IN-42 §5 P2). `EdgeWorker.handleSessionStartMessage` / `handleUserPromptMessage` are no longer TODO placeholders: they compute a side-effect-free routing "parity" snapshot from the `SessionCorrelationRegistry` and, in the default **shadow** mode, only log it while the legacy `event` path keeps owning execution (behavior unchanged). A new `CYRUS_BUS_SESSION_OWNERSHIP` env flag (`shadow`/`off` default, `active`/`all`/`on`, or a comma list of sources like `linear`) opts a source into **active** ("switch") mode: the legacy `handleWebhook` hands the raw Linear webhook off to the bus and early-returns, and the bus consumes it to drive the exact same, fully-tested `handleAgentSessionCreatedWebhook` / `handleUserPromptedAgentActivity` — so a runner starts exactly once (no double-start) without reimplementing routing/access-control/parking. Stop signals are never handed to the bus (their handler is still a placeholder) so they can't be dropped, and a bounded stash sweep guards against leaks. Follows the `handleIssueStateChangeMessage` "bus takes over + legacy early-return" precedent and stays fully reversible via the flag. ([IN-47](https://linear.app/principle-intl/issue/IN-47), [#19](https://github.com/yuanze-dev/cyrus/pull/19))
- Chat session storage converged onto the singleton `AgentSessionManager` (IN-42 §5 P1). `ChatSessionHandler` no longer constructs its own private, memory-only `AgentSessionManager` — the shared singleton and the `SessionCorrelationRegistry` are injected via deps, so Feishu/Slack chat sessions serialize/restore with the rest of EdgeWorker state and survive restarts. `CyrusAgentSession` gains an optional, discriminated `channels?: ChannelBinding[]` (`linear`/`feishu`/`slack`); adapters populate it on create via a new optional `getChannelBinding` hook, and `AgentSessionManager.restoreState` backfills a `linear` binding for old sessions that predate the field. On create the handler binds the thread's canonical + alias keys into the correlation registry; `resolveThreadSession` falls back to that persisted registry (guarded on the session actually surviving restore) so a same-thread follow-up after a restart re-merges to the original session, and the resume branch now continues a session with no live runner via `--continue` instead of forking a fresh one. EdgeWorker aggregators (`getAllKnownSessions`, shutdown runner collection, `computeStatus`) were de-duplicated since chat sessions/runners now live in the singleton, and the handler's `getAllChatSessions`/`getAllRunners`/`isAnyRunnerBusy` are scoped to owned session ids. **Migration note:** the design's staged "dual-write private+singleton → diff → delete" gray step was collapsed into a direct convergence — shipping throwaway comparison scaffolding added risk without value, so equivalence is instead pinned by tests (existing three-state follow-up suites now run through the singleton, plus new serialize→restore/re-merge/backfill coverage). ([IN-46](https://linear.app/principle-intl/issue/IN-46), [#18](https://github.com/yuanze-dev/cyrus/pull/18))
- Session correlation base for the unified Session · multi-Channel effort (IN-42 §5 P0). `GlobalSessionRegistry` is upgraded to `SessionCorrelationRegistry` (old name kept as a backward-compatible alias) with a `channelKey → sessionId` index (`bind`/`resolve`/`unbind`/`getChannelKeysForSession`) that lives alongside the existing child→parent map and is persisted as `sessionChannelIndex` in the EdgeWorker state. Internal messages now carry optional `sessionKeyAliases`, and the Feishu translator emits them (`chatId:rootId` / `chatId:messageId`) so a thread whose primary key shifts from `chatId:messageId` to `chatId:threadId` mid-flight reconciles to one session. `EdgeWorker.handleMessage` shadow-records these correlations without touching the legacy `event` path (behavior unchanged). ([IN-45](https://linear.app/principle-intl/issue/IN-45), [#17](https://github.com/yuanze-dev/cyrus/pull/17))
