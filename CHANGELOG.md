...
### Added
- You can now allow- or block-list Feishu (Lark) users by their `open_id`. Access control previously only understood Linear identities (id/email); add `{ "openId": "ou_…" }` entries to `userAccessControl.allowedUsers` / `blockedUsers` (globally or per repository) to gate who may drive Cyrus from Feishu. Blocked users are ignored silently, and an allowlist that contains only `open_id`s still restricts Feishu to exactly those people. Linear-side gating is unchanged. ([IN-50](https://linear.app/principle-intl/issue/IN-50), [#23](https://github.com/yuanze-dev/cyrus/pull/23))

### Fixed
- A Feishu (Lark) event delivered over both the long connection and the webhook at once is no longer acted on twice. The two receive paths previously kept separate "already seen" memories, so the same message could start or continue a session twice; they now share one de-duplication window. Cyrus also ignores a message that merely echoes a notice it just posted into the same thread (or an immediate duplicate re-delivery), so a completion/progress reply can never loop back and spin up a brand-new session. ([IN-50](https://linear.app/principle-intl/issue/IN-50), [#23](https://github.com/yuanze-dev/cyrus/pull/23))

### Added
- Feishu (Lark) tasks can now stream **progress** back into their thread, not just a final "done" notice. When a Feishu thread creates a Linear issue, Cyrus can reply in that same thread at each milestone — the agent's answer at the end of a turn, failures, and cancellation — so you can follow along in Feishu without switching to Linear. It only posts milestones (never every intermediate thought or tool step), never repeats the same update, and won't loop on its own messages. Off by default; enable it by setting `CYRUS_FEISHU_BACKFLOW` to `on` (or `true`/`active`/`all`/`feishu`). With it off, behavior is unchanged — you still get exactly the single completion notice when the issue is marked Done. ([IN-49](https://linear.app/principle-intl/issue/IN-49))
- Feishu (Lark) and Slack chat conversations now survive a Cyrus restart. Previously a chat session lived only in memory, so restarting Cyrus lost the thread — a follow-up in the same Feishu topic or Slack thread started over with no memory of the earlier exchange. Chat sessions are now persisted alongside everything else, and when you follow up in the same thread after a restart Cyrus reconnects to the original conversation (continuing it with `--continue`) instead of spawning a fresh, empty one. No configuration needed. ([IN-46](https://linear.app/principle-intl/issue/IN-46), [#18](https://github.com/yuanze-dev/cyrus/pull/18))
- Feishu (Lark) threads can now choose which engine (Claude or Codex) answers them. Start a message with `/claude` or `/codex` (case-insensitive) to run that engine for a brand-new topic — the prefix is stripped before the model sees your text, so `/codex 帮我…` reaches the model as just "帮我…". Because each engine keeps its own conversation state, the choice is locked once a topic starts: sending a prefix mid-thread doesn't switch engines, and Cyrus replies telling you to open a new topic to change. Without a prefix, Cyrus picks the engine from optional per-user (`feishuUserRunners`) and per-chat (`feishuChatRunners`) mappings in config, then falls back to the global `defaultRunner`; a per-user setting wins over a per-chat one. When a Feishu session creates a Linear issue, the issue description carries the same engine's `[agent=…]` tag so the follow-up Linear session runs on the matching engine — deterministically for Claude (injected if the model didn't add one) and, for Codex, via a prompt hint plus a parentage fallback that remembers the originating engine even when the tag is missing. ([IN-38](https://linear.app/principle-intl/issue/IN-38), [#12](https://github.com/yuanze-dev/cyrus/pull/12))
- Cyrus can now see images you send it in Feishu (Lark). Attach a screenshot (or send an image, or mix images and text in a rich-text post) and @mention Cyrus, and it downloads the picture and actually looks at it when answering — instead of receiving an empty message and missing the image entirely. Works for a plain image, an image sent together with text, and @mentioning Cyrus with an image attached. Requires the Feishu app to have the message-resource read permission (`im:resource`); if an image can't be downloaded (missing permission or a transient error), Cyrus tells you it couldn't view it and keeps handling the text rather than failing. ([IN-34](https://linear.app/principle-intl/issue/IN-34), [#11](https://github.com/yuanze-dev/cyrus/pull/11))
- Cyrus can now read Feishu (Lark) multi-dimensional tables (Bitable / 多维表格 / base). Paste a base link (`https://<tenant>.feishu.cn/base/<token>`) in a Feishu thread or Linear issue and Cyrus reads the table's structured content — its data tables, fields and records — instead of returning "not supported". A base with several tables is listed and read in one call; append `?table=<tableId>` to the link (or ask for a specific table) to read just one, and rows per table are capped (100 by default, up to 500) to keep responses manageable. Bitable pages stored in a wiki are read too. If the app can't see the base you get a clear permission error telling you to share the base with the Cyrus bot. Requires the app to have `bitable:app:readonly` permission. (Standalone Sheets / 电子表格 are still not read and return a note.) ([IN-16](https://linear.app/principle-intl/issue/IN-16))
- Feishu (Lark) tasks now get a completion notice back in their thread. When you ask Cyrus in a Feishu thread to make repository changes, it creates a Linear issue for the work — and once that issue is marked Done in Linear, Cyrus replies right in the original Feishu thread with the task title and a clickable Linear link, addressed to whoever requested it. The notice stays inside the topic (no extra pings), fires only once per issue, and survives a Cyrus restart because the thread↔issue link is persisted. Only Done triggers a notice (canceled issues don't), and only issues that originated from Feishu are tracked. ([IN-10](https://linear.app/principle-intl/issue/IN-10), [#4](https://github.com/yuanze-dev/cyrus/pull/4))
- Cyrus now shows real names instead of opaque Feishu (Lark) `open_id`s. When someone @mentions Cyrus in Feishu, it looks up the sender's display name so the agent knows who is asking ("Alice Wang" instead of `ou_ea1fe5f5…`), and thread/replied-to context shows each participant's name too. Requires the app to have contact-read permission (`contact:user.base:readonly`); without it, Cyrus silently falls back to the bare open_id and keeps working. Names are cached so each id is only looked up once per process. ([FP-292](https://linear.app/principle-intl/issue/FP-292), [#3](https://github.com/yuanze-dev/cyrus/pull/3))
- Feishu (Lark) sessions can now optionally run as a full-capability agent instead of the default read-only chat mode. Set `FEISHU_FULL_ACCESS=true` to give Feishu sessions the complete tool set (Write/Edit/run shell commands) and unrestricted access to the host filesystem (including `~/.cyrus` and the home directory), so the agent can install skills, inspect config, query data on the server, and carry out operational tasks directly rather than only planning them or routing everything to a Linear issue. Off by default, and it only affects Feishu — Slack sessions stay read-only. **Security:** with this enabled, anyone who can message the bot can run arbitrary commands as the host user, so only turn it on for trusted, private Feishu channels.
- Cyrus can now read Feishu (Lark) documents. When a message includes a Feishu doc link (a `docx` document or a `wiki` page), the agent reads it through the Feishu API with the app's own credentials instead of trying to fetch the URL (which fails on Feishu's login wall). Enabled automatically when `FEISHU_APP_ID`/`FEISHU_APP_SECRET` are set. Requires the app to have `docx:document:readonly` + `wiki:wiki:readonly` (or `wiki:node:read`) permissions, and the specific document to be shared with the Cyrus bot. (Bitable/base support was added separately — see above; standalone Sheets still return a note.) ([#2](https://github.com/AirboZH/cyrus/pull/2))
- Feishu (Lark) is now supported as a chat front door, alongside Slack. @mention Cyrus in a Feishu group (or message it directly) to kick off work: Cyrus opens a session, and when you ask for repository changes it creates a Linear issue and replies with status right in the Feishu thread — no need to leave Feishu. Set `FEISHU_APP_ID` and `FEISHU_APP_SECRET` to enable it. Two ways to receive events: **long connection (recommended for self-host)** — Cyrus connects out to Feishu over a WebSocket using the official Feishu SDK, so **no public callback URL, Encrypt Key, or Verification Token is needed** (just enable the app's long-connection event subscription); or **webhook** — point the app's event subscription at `/feishu-webhook` and set `FEISHU_VERIFICATION_TOKEN` (+ optional `FEISHU_ENCRYPT_KEY`). Set `FEISHU_BASE_URL` to target Lark international (`https://open.larksuite.com/open-apis`); it defaults to `https://open.feishu.cn/open-apis`. Automatic thread-following can be turned off with `CYRUS_FEISHU_THREAD_FOLLOWING_DISABLED=true`. ([#1](https://github.com/AirboZH/cyrus/pull/1))

### Fixed
- The Feishu (Lark) `/claude`·`/codex` engine prefix now works in group chats, not just direct messages. In a group you have to @mention Cyrus to reach it, and that `@Cyrus` was landing in front of the prefix — so `@Cyrus /codex 帮我…` wasn't recognized as an engine switch, didn't get stripped, and was passed to Claude verbatim, which reported "Unknown command". Cyrus now removes its own @mention before reading the prefix, so a brand-new topic starts on the requested engine (the model sees just "帮我…"), and sending a prefix inside an existing topic gets the usual "this topic is locked to engine X, open a new topic to switch" reply instead of leaking `/codex` to the model. Direct messages are unchanged, and an @mention with no prefix no longer leaves a stray `@Cyrus` in the text. ([IN-39](https://linear.app/principle-intl/issue/IN-39), [#13](https://github.com/yuanze-dev/cyrus/pull/13))
- Feishu (Lark) follow-ups no longer split into a brand-new, empty conversation. When Cyrus asked a question in a Feishu thread and you answered, your answer could land in a fresh session that had lost all context ("this is a new session, I didn't keep your earlier questions"). Cyrus now identifies a conversation by its Feishu topic (`thread_id`) when there is one, and reconciles the earlier messages — which may have been keyed on the reply-root or message id before the topic existed — back to the same session, so your reply continues the original conversation and Cyrus remembers what it asked. As an extra safety net, if an answer still slips into a new session, Cyrus pulls in its most recent reply from that same chat so it can pick up the thread instead of claiming it lost context. ([IN-27](https://linear.app/principle-intl/issue/IN-27))
- Cyrus no longer reacts to messages it wasn't addressed in. In any chat channel/group where the bot is present (Feishu or Slack), plain messages that don't @mention Cyrus — and aren't part of a conversation Cyrus is already in — used to get an "OnIt" emoji receipt anyway, even though Cyrus then correctly stayed silent. Now Cyrus adds no reaction and takes no action on those messages; it only reacts when you @mention it or when you continue a thread it's already working in. ([IN-13](https://linear.app/principle-intl/issue/IN-13), [#5](https://github.com/yuanze-dev/cyrus/pull/5))
- When you reply to a specific Feishu (Lark) message and @mention Cyrus (e.g. someone posts a request, then you reply to it with "@Cyrus do this"), Cyrus now reads the message you replied to and uses it as context. Previously it only saw your reply text and missed the original message, so it had no idea what you were referring to. Requires the Feishu app to have message-read permission.
...
### Added
- Slack thread-following can now be turned off. It's on by default; disable it per-team from the Behaviours settings page, or force it off on a self-hosted instance by setting the `CYRUS_SLACK_THREAD_FOLLOWING_DISABLED` environment variable (the env variable always wins). When off, Cyrus only responds to explicit @mentions. On managed teams, asking Cyrus in a thread to stop listening now gets a link to the Behaviours page, and Cyrus goes quiet in that thread until someone asks it a direct question. ([CYPACK-1267](https://linear.app/ceedar/issue/CYPACK-1267), [#1273](https://github.com/cyrusagents/cyrus/pull/1273))
- Slack messages now show when Cyrus has finished processing them: the 👀 reaction added on receipt is swapped for ✅ once the turn completes — including when Cyrus decided the message didn't need a reply, so you can tell "seen but skipped" apart from "still working". ([CYPACK-1267](https://linear.app/ceedar/issue/CYPACK-1267), [#1273](https://github.com/cyrusagents/cyrus/pull/1273))
