...
- Cyrus can now read Feishu (Lark) documents. When a message includes a Feishu doc link (a `docx` document or a `wiki` page), the agent reads it through the Feishu API with the app's own credentials instead of trying to fetch the URL (which fails on Feishu's login wall). Enabled automatically when `FEISHU_APP_ID`/`FEISHU_APP_SECRET` are set. Requires the app to have `docx:document:readonly` + `wiki:wiki:readonly` (or `wiki:node:read`) permissions, and the specific document to be shared with the Cyrus bot. Sheets and Base (bitable) are not read yet — the tool returns a note for those. ([#2](https://github.com/AirboZH/cyrus/pull/2))
