...
### Fixed
- Codex, Gemini, and Cursor sessions now follow the global operating rules. Cyrus keeps its top-level hard constraints (repository routing, ship/auto-merge whitelist, commit-attribution rules, etc.) in a global `CLAUDE.md`, but only Claude ever read it — the other engines each look for their own memory file (`AGENTS.md`, `GEMINI.md`, or none) and silently missed those rules, so switching an issue to Codex/Gemini/Cursor dropped the global playbook. Those rules are now injected into the system prompt for every non-Claude engine as the highest-priority block, so behavior stays consistent whichever engine runs the issue. As part of this, Cursor issue sessions — which previously discarded the system prompt entirely — now receive it. ([IN-43](https://linear.app/principle-intl/issue/IN-43), [#21](https://github.com/yuanze-dev/cyrus/pull/21))
